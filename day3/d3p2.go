...
func set_val_adj_to_gear(col_idx_of_gear int, numbers []*number, gear *gear) {
    for _, n := range numbers {
        if col_idx_of_gear >= n.xStart-1 && col_idx_of_gear <= n.xEnd+1 {
            gear.val = append(gear.val, n.val)
        }
    }
}
...
`
    ...
    for row_idx, rLine := range matrix {
        for col_idx, char := range rLine {
            g := gear{row_idx: row_idx, col_idx: col_idx}
            if (char == '*') {
                if row_idx > 0 {
                    // mark above row
                    set_val_adj_to_gear(col_idx, numbersPerRow[row_idx-1], &g)
                }
                // mark current row
                set_val_adj_to_gear(col_idx, numbersPerRow[row_idx], &g)
