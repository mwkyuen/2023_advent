...
func get_hand_value(rcard []rune) handValue {

    counter := make(map[rune]int) 
    for _, char := range rcard {
        counter[char]++
    }
    ...
    if ok {
        ...
        if curr_max_v == 1 && len(counter) == 4 {
            return one_pair
        } else if curr_max_v == 1 && len(counter) == 3 {
            return three_of_a_kind
        } else if curr_max_v == 1 && len(counter) == 2 {
            return four_of_a_kind
        } else if curr_max_v == 1 && len(counter) == 1 {
            return five_of_a_kind
        } else {
            counter[curr_max_k] = curr_max_v + j_val
        }
...
func is_still_bigger(r1 []rune, r2 []rune) bool {
    // r1 := []rune(card1)
    // r2 := []rune(card2)
    for i:=0; i < len(r1); i++ {
...
func is_bigger(card1 string, card2 string) bool {
    r1 := []rune(card1)
    r2 := []rune(card2)
    hand_value_1 := get_hand_value(r1)
    hand_value_2 := get_hand_value(r2)
    if (hand_value_1 == hand_value_2) {
        return is_still_bigger(r1, r2)
    } else {
